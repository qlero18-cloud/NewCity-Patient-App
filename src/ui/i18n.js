...
export const STRINGS = {
  es: {
    ...
    stay: {
      ...
      // Etapa L (D101) — los cuatro que venían en el Word y se tiraban.
    ...
    coordinator: {
      ...
      import: {
        ...
        meetingPointPlaceholder: 'Elige el punto de encuentro',
        vehicleTypePlaceholder: 'Elige el tipo',
        ...
        note: {
          ...
          // Etapa L — los del intérprete de reservas. Cada uno nombra el
          // renglón del Word del que salió: la coordinadora tiene el
          // documento abierto al lado y "no pudimos leerlo" a secas la deja
          // buscando cuál de doce campos es.
          templateBlank: (n) => `El documento dejó “${n.text}” sin llenar: no se importa nada ahí.`,
          unknownLabel: (n) => `No supimos a qué campo va este renglón: ${n.text}`,
          valueTooLong: (n) => `“${n.text}” es demasiado largo. Recórtalo.`,
          valueUnreadable: (n) => `No pudimos leer ${n.from}: ${n.text}`,
          noYear: 'El documento no trae el año, así que la fecha quedó vacía.',
          dateUnreadable: (n) => `No pudimos leer la fecha: ${n.text}`,
          timeAssumed: (n) => `Hora supuesta en ${n.from}: ${n.to}. El documento no la dice.`,
          timeMissing: (n) => `El documento no dice la hora de ${n.text}.`,
          dateMissing: 'El documento no dice cuándo es este traslado.',
          nightsMismatch: (n) => `El documento dice ${n.from} noches, pero entre las fechas hay ${n.to}.`,
          guestMismatch: (n) => `El hotel está a nombre de ${n.from} y el itinerario dice ${n.to}.`,
          roundTrip: 'Viaje redondo: se proponen dos traslados, llegada y regreso.',
          roundTripIncomplete: 'Dice viaje redondo, pero solo trae una fecha.',
          transferTypeUnknown: (n) => `No reconocimos el tipo de traslado: ${n.text}`,
          meetingPointUnknown: (n) => `No reconocimos el punto de encuentro: ${n.text}`,
          meetingPointMissing: 'Falta el punto de encuentro: el documento no lo dice.',
          countryCodeAssumed: (n) => `Le pusimos lada al teléfono: ${n.from} → ${n.to}. Confírmala.`,
          phoneUnreadable: (n) => `No pudimos leer el teléfono: ${n.text}`,
          vehicleSplit: (n) => `“${n.text}” lo partimos en marca y modelo.`,
          vehicleTypeMissing: 'Falta el tipo de vehículo: elígelo, no lo deducimos del nombre comercial.',
          pickupBeforeCheckIn: (n) => `La recogida es el ${n.to}, antes del check-in del ${n.from}.`,
        },
        warning: {
      ...
      lodging: {
        ...
        // Etapa L (D101). El total se teclea con moneda y todo —"$3,164.00
        // MXN"— porque se guarda verbatim: un campo numérico obligaría a
        // tirar el "MXN" y a que alguien adivine después en qué moneda es.
        roomTypeLabel: 'Tipo de habitación',
        nightsLabel: 'Noches',
        occupancyLabel: 'Ocupación',
        totalLabel: 'Total (con moneda)',
  ...
  en: {
    ...
    coordinator: {
      ...
      import: {
        ...
        meetingPointPlaceholder: 'Choose a meeting point',
        ...
        note: {
          ...
          templateBlank: (n) => `The document left “${n.text}” unfilled: nothing is imported there.`,
          unknownLabel: (n) => `We could not tell which field this row belongs to: ${n.text}`,
          valueTooLong: (n) => `“${n.text}” is too long. Shorten it.`,
          valueUnreadable: (n) => `We could not read ${n.from}: ${n.text}`,
          noYear: 'The document has no year, so the date came out empty.',
          dateUnreadable: (n) => `We could not read the date: ${n.text}`,
          timeAssumed: (n) => `Assumed time for ${n.from}: ${n.to}. The document does not say.`,
          timeMissing: (n) => `The document does not give a time for ${n.text}.`,
          dateMissing: 'The document does not say when this transfer happens.',
          nightsMismatch: (n) => `The document says ${n.from} nights, but the dates are ${n.to} apart.`,
          guestMismatch: (n) => `The hotel is booked under ${n.from} and the itinerary says ${n.to}.`,
          roundTrip: 'Round trip: we propose two transfers, arrival and departure.',
          roundTripIncomplete: 'It says round trip, but only one date is given.',
          transferTypeUnknown: (n) => `We did not recognize the transfer type: ${n.text}`,
          meetingPointUnknown: (n) => `We did not recognize the meeting point: ${n.text}`,
          meetingPointMissing: 'Meeting point missing: the document does not say.',
          countryCodeAssumed: (n) => `We added a country code to the phone: ${n.from} → ${n.to}. Confirm it.`,
          phoneUnreadable: (n) => `We could not read the phone number: ${n.text}`,
          vehicleSplit: (n) => `We split “${n.text}” into make and model.`,
          vehicleTypeMissing: 'Vehicle type missing: pick it — we do not infer it from the brand name.',
          pickupBeforeCheckIn: (n) => `Pickup is on ${n.to}, before the ${n.from} check-in.`,
        },
        warning: {
      ...
      lodging: {
        ...
        totalLabel: 'Total (with currency)',
        save: 'Save lodging',
        saved: 'Lodging saved.',
        error: {
          required: 'This field is required.',
